...
Tree* Set::_Insert(Tree* root, int key, bool &f)
{
    if (root == nullptr) {
        root = new Tree(key);
        f = true;
    ...
    {
        root->left = _Insert(root->left, key, f);
    }
    else if (key > root->key)
    {
        root->right = _Insert(root->right, key, f);
    }
    else if (key == root->key) {
        f = false;
    }
...
Tree* Set::_Erase_elem(Tree* root, int key, bool &f)
{
    if (!root)
        return nullptr;
    if (key < root->key)
    {
        root->left = _Erase_elem(root->left, key, f);
    }
    else if (key > root->key)
    {
        root->right = _Erase_elem(root->right, key, f);
    }
    else
    {
...
{
    bool f = true;
    Tree* new_root = _Erase_elem(root, key, f);
    if (new_root == nullptr) {
        return false;
    }
    root = new_root;
